...
function robo_trade_sell_BTC_BBL(){
	Volume.findOne({'MarketName' : 'BTC-LEC'},function(err,volume){
		if (volume)
		{
			var last = parseFloat(volume.last);
			var quantity = parseInt(_.random(100,1000));
			var rand_price_add  = parseInt(_.random(10,50));
			...
			{
				newOrderSell("5a1a3dab23b6e474a9e4324d", "BTC-LEC",quantity*100000000,parseFloat(last)+parseFloat(rand_price_add), subtotal, commission, subtotal).save(( err,order_create)=>{
					order_create && matching_buy(order_create);        		
					setTimeout(function() {
						newOrderBuy("5a1a3dab23b6e474a9e4324d", "BTC-LEC",quantity*100000000,parseFloat(last)+parseFloat(rand_price_add), subtotal, commission, subtotal).save(( err,order_createdd)=>{
...
function robo_trade_sell_BCC_BBL(){

	Volume.findOne({'MarketName' : 'BCC-BBL'},function(err,volume){
		if (volume)
		{
			/*Ticker.findOne({},function(errssss,resultsss){
				if (!errssss && resultsss)
				{
					var last = parseFloat(volume.last);
					var quantity = parseInt(_.random(100,1000));
					var rand_price_add  = parseInt(_.random(10,50));
					...
					var commission = (subtotal*0.0025).toFixed(8);
					var randomss = _.random(1,2);
					console.log(parseFloat(resultsss.bcc.btc)*100000000,parseFloat(volume.last));
					if (parseFloat(resultsss.bcc.btc)*100000000 >= parseFloat(volume.last))
					{
						newOrderBuy("5a1a3dab23b6e474a9e4324d", "BCC-BBL",quantity*100000000,parseFloat(last)-parseFloat(rand_price_add), subtotal, commission, subtotal).save(( err,order_createdd)=>{
							order_createdd && matching_sell(order_createdd);
						})
					}
					else
					{
						newOrderSell("5a1a3dab23b6e474a9e4324d", "BCC-BBL",quantity*100000000,parseFloat(last)+parseFloat(rand_price_add), subtotal, commission, subtotal).save(( err,order_create)=>{
							order_create && matching_buy(order_create);        		
						})
					}
				}
...
function initssss() {
    var myFunction = function() {
        ...
        var rand = Math.round(Math.random() * (25000 - 500)) + 5000; 
