...
function get_coin_details(name,callback){
	...
	if (name === 'LEC') { data.confirmations = 0,  data.free = 0.001, data.client =  STCclient };
	callback(data);
}
...
function Auto_Confirm_Withdraw(){
	withdraw('LEC',STCclient,'',function(cb){
		cb ? console.log('Send Success LEC') : console.log('Send Fail LEC')
	})
	sleep.sleep(1);
