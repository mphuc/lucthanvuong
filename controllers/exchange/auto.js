...
cron.schedule('*/5 * * * *', function(){ // 5 min chart 6h
  auto_push_chart('BTC-LEC','6h');
});

cron.schedule('*/20 * * * *', function(){ // 20 min chart 24h
  auto_push_chart('BTC-LEC','24h');
});

cron.schedule('0 */1 * * *', function(){ // 60 min chart 3d
  auto_push_chart('BTC-LEC','3d');
});

cron.schedule('0 */2 * * *', function(){ // 140 min chart 1w
  auto_push_chart('BTC-LEC','1w');
});

cron.schedule('0 */4 * * *', function(){ // 280 min chart 2w
  auto_push_chart('BTC-LEC','2w');
});

//auto_push_chart_60('BTC-BBL');
Create_itemchart('BTC-LEC');
		...
		var data_update = {
			$push: {
	            'history6h': {
	                ...
	                'hight': hight,
	                'low': low,
	                'volume': (0.1 + parseFloat(Math.random())/100000).toFixed(8),
	                'date': new Date(date_new).getTime() * 1000,
	                'created_on' : dateFormat(new Date(date_new).toLocaleString(), "yyyy-mm-dd h:MM:ss")
	            }
	        }
		};
		Chart.update(query, data_update, function(err, newUser) {
		...
		var data_update = {
			$push: {
	            'history24h': {
	                ...
	                'hight': hight,
	                'low': low,
	                'volume': parseFloat(Math.random()/1000).toFixed(8),
	                'date': new Date(date_new).getTime() * 1000,
	                'created_on' : dateFormat(new Date(date_new).toLocaleString(), "yyyy-mm-dd h:MM:ss")
	            }
	        }
						...
						data_update = {
							$push: {
				                history6h: {
				                    ...
				                    'volume': (parseFloat(volume))/100000000,
				                    'date': new Date().getTime() * 1000,
				                    'created_on' : dateFormat(new Date().toLocaleString(), "yyyy-mm-dd h:MM:ss")
				                }
				            }
						}
					),
